...
class nodules_crop(object):
    ...
    def __init__(self, workspace):
        ...
        # ls_all_patients存放对应所有病人的mhd文件名,一个病人的数据是一个序列,在一个mhd文件中
        self.ls_all_patients = glob(self.all_patients_path + "*.mhd")
        # 读取自annotations.csv中的数据,注意这里的结节都是分类为1的结节,而且能读取到结节的直径
        ...
        # 读取自candidates.csv的数据,注意这里是所有类型结节的集合,能读取到结节的二分类值
