...
def main():
    ...
    main_app = LIDCInterfaceTk(None, lung_ct_path="G:/DataSet/TCIA_LIDC-IDRI/LIDC-IDRI")
        ...
        for trial in [1,2]:
            ...
            for ct in patient_ct_series.annotated_cts:
                ...
                if ct.no_consensus_annots() > 0: #if one or more rads agree
                    ...
                    cv_mat = ct.draw(rad_indx_lst=list(range(len(xml_dt.rad_annotations))), 
...
def main_tmp2():
   ...
   # print xml_dt
    ...
    for ct in patient_series.annotated_cts:
        rad_indx = list(range(len(xml_dt.rad_annotations))) # is from 0 to 3
